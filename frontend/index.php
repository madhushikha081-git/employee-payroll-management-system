...
if ($alreadyExists) {
    ...
    $sql = "
        ...
        SELECT
            ...
            ROUND(
                (ss.basic_salary + ss.hra + ss.travel_allowance + ss.other_allowance)
                * SUM(
                    CASE a.attendance_status
                        WHEN 'PRESENT' THEN 1
                        WHEN 'HALF_DAY' THEN 0.5
            ...
            ROUND(
                (ss.pf_deduction + ss.tax_deduction)
                * SUM(
                    CASE a.attendance_status
                        WHEN 'PRESENT' THEN 1
                        WHEN 'HALF_DAY' THEN 0.5
        ...
        FROM employee e
        JOIN salary_structure ss
            ON ss.employee_id = e.employee_id
            AND ss.effective_to IS NULL
        JOIN attendance a
            ON a.employee_id = e.employee_id
            AND DATE_FORMAT(a.attendance_date, '%Y-%m') = DATE_FORMAT(?, '%Y-%m')
        WHERE e.employee_id = ?
        GROUP BY
            e.employee_id,
            ss.salary_id,
